...
def convergence_study(f, y00, a, b, h_values, exact_solution):
    ...
    plt.title('Convergence Study of RK 4')
    plt.grid()
    plt.legend()
    plt.show()
...
def plot(f, y00, a, b, h_values):
    ...
    for h in h_values:
        ...
        plt.plot(u_values[:,0], u_values[:,2], label=f"h={h}", marker='^', markersize=1, linewidth=1)

    plt.xlabel('x(t)')
    plt.ylabel('y(t)')
    plt.title('x(t), y(t) for different stepsize h')
    plt.legend()
    plt.grid()
    plt.show()
